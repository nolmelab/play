...
TEST_CASE("ensure")
{
  ...
  SUBCASE("flow and jump")
  {
    ...
    mockup_actor owner;
    act_ensure::ptr fp = std::make_shared<flow>(owner, jflow);
  ...
  SUBCASE("act::path")
  {
    SUBCASE("basics")
    {
      ...
      CHECK(act_ensure::path::is_absolute_path(p1.full_path_));
      CHECK(!act_ensure::path::is_absolute_path(p1.act_name_));
      CHECK(act_ensure::path::is_relative_path(p1.act_name_));
      CHECK(act_ensure::path::get_first_act(p1.full_path_) == "a");
      CHECK(act_ensure::path::get_last_act(p1.full_path_) == "b");
      CHECK(act_ensure::path::pop_head_act(p1.full_path_) == "b");
    }
    ...
    SUBCASE("child")
    {
      ...
      act_ensure::path p2;
      p2.setup("/aa/bb");
      CHECK(act_ensure::path::is_child_of(p2.full_path_, p1.full_path_));
      CHECK(act_ensure::path::get_child_path(p2.full_path_, p1.full_path_) == "cc/dd");
    }
  }
}
