...
#include <play/robust/db/nanodbc.h>
...
inline std::string recent_error(SQLHANDLE handle, SQLSMALLINT handle_type, long& native,
                                std::string& state)
{
  ...
  do
  {
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDiagRec), rc, handle_type, handle, (SQLSMALLINT)i, sql_state,
                    &native_error, nullptr, 0, &total_bytes);
    ...
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDiagRec), rc, handle_type, handle, (SQLSMALLINT)i, sql_state,
                    &native_error, sql_message.data(), (SQLSMALLINT)sql_message.size(),
                    &total_bytes);
...
#else
#define NANODBC_THROW_DATABASE_ERROR(handle, handle_type) \
  throw nanodbc::database_error(handle, handle_type,      \
...
template <typename T>
using is_integral8 = std::integral_constant<bool, std::is_integral<T>::value && sizeof(T) == 1 &&
                                                      !std::is_same<T, char>::value>;

template <typename T>
using is_integral16 = std::integral_constant<bool, std::is_integral<T>::value && sizeof(T) == 2 &&
                                                       !std::is_same<T, wchar_t>::value>;

template <typename T>
using is_integral32 = std::integral_constant<bool, std::is_integral<T>::value && sizeof(T) == 4 &&
                                                       !std::is_same<T, wchar_t>::value>;
...
template <typename T>
struct sql_ctype<
    T, typename std::enable_if<is_integral16<T>::value && std::is_unsigned<T>::value>::type>
{
...
template <typename T>
struct sql_ctype<
    T, typename std::enable_if<is_integral32<T>::value && std::is_unsigned<T>::value>::type>
{
...
template <typename T>
struct sql_ctype<
    T, typename std::enable_if<is_integral64<T>::value && std::is_unsigned<T>::value>::type>
{
...
inline void allocate_env_handle(SQLHENV& env)
{
  ...
  try
  {
    NANODBC_CALL_RC(SQLSetEnvAttr, rc, env, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)NANODBC_ODBC_VERSION,
                    SQL_IS_UINTEGER);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(env, SQL_HANDLE_ENV);
  }
...
namespace nanodbc {
connection::attribute::attribute(long const& attribute, long const& string_length,
...
void connection::attribute::extractValuePtr()
{
  std::visit(
      [this](auto&& arg)
      {
        ...
        else if constexpr (std::is_same_v<T, std::intptr_t> || std::is_same_v<T, std::uintptr_t>)
        {
...
public:
...
#ifdef NANODBC_HAS_STD_VARIANT
  connection_impl(string const& dsn, string const& user, string const& pass,
  ...
  void enable_async(void* event_handle)
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(SQLSetConnectAttr, rc, dbc_, SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE,
                    (SQLPOINTER)SQL_ASYNC_DBC_ENABLE_ON, SQL_IS_INTEGER);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);

    NANODBC_CALL_RC(SQLSetConnectAttr, rc, dbc_, SQL_ATTR_ASYNC_DBC_EVENT, event_handle,
                    SQL_IS_POINTER);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
  }
  ...
  RETCODE connect(string const& dsn, string const& user, string const& pass, long timeout,
                  void* event_handle = nullptr)
  {
    ...
    if (timeout != 0)
    {
      attributes.push_back({SQL_ATTR_LOGIN_TIMEOUT, SQL_IS_UINTEGER, (std::uintptr_t)timeout});
    }
#if !defined(NANODBC_DISABLE_ASYNC) && defined(SQL_ATTR_ASYNC_DBC_EVENT)
    if (event_handle != nullptr)
    {
      attributes.push_back({SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE, SQL_IS_UINTEGER,
  ...
  RETCODE
  connect(string const& dsn, string const& user, string const& pass,
          std::list<attribute> const& attributes)
  {
    ...
    NANODBC_CALL_RC(NANODBC_FUNC(SQLConnect), rc, dbc_, (NANODBC_SQLCHAR*)dsn.c_str(), SQL_NTS,
                    !user.empty() ? (NANODBC_SQLCHAR*)user.c_str() : nullptr, SQL_NTS,
                    !pass.empty() ? (NANODBC_SQLCHAR*)pass.c_str() : nullptr, SQL_NTS);
  ...
  connect(string const& connection_string, std::list<attribute> const& attributes)
  {
    ...
    NANODBC_CALL_RC(NANODBC_FUNC(SQLDriverConnect), rc, dbc_, nullptr,
  ...
  string catalog_name() const
  {
    ...
    SQLINTEGER length(0);
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetConnectAttr), rc, dbc_, SQL_ATTR_CURRENT_CATALOG, name,
...
string connection::connection_impl::get_info_impl<string>(short info_type) const
{
  ...
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLGetInfo), rc, dbc_, info_type, value,
                  sizeof(value) / sizeof(NANODBC_SQLCHAR), &length);
  if (!success(rc))
    NANODBC_THROW_DATABASE_ERROR(dbc_, SQL_HANDLE_DBC);
...
public:
        ...
        committed_(false)
  {
    ...
    {
      RETCODE rc;
      NANODBC_CALL_RC(SQLSetConnectAttr, rc, conn_.native_dbc_handle(), SQL_ATTR_AUTOCOMMIT,
  ...
  ~transaction_impl() noexcept
  {
    try
    {
      ...
      if (conn_.transactions() == 0 && conn_.connected())
      {
        ...
        NANODBC_CALL(SQLSetConnectAttr, conn_.native_dbc_handle(), SQL_ATTR_AUTOCOMMIT,
...
public:
...
#ifndef NANODBC_DISABLE_MSSQL_TVP
  void open_tvp(table_valued_parameter& tvp, short param_index, bound_parameter& param,
  ...
  RETCODE prepare(string const& query, long timeout, void* event_handle = nullptr)
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLPrepare), rc, stmt_, (NANODBC_SQLCHAR*)query.c_str(),
  ...
  {
    RETCODE rc;
    NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_QUERY_TIMEOUT,
  ...
  void enable_async(void* event_handle)
  {
    ...
    if (!async_enabled_)
    {
      NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_ENABLE,
    ...
    {
      NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_STMT_EVENT, event_handle,
                      SQL_IS_POINTER);
      if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
  ...
  void disable_async() const
  {
    if (async_enabled_)
    {
      RETCODE rc;
      NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_ENABLE,
                      (SQLPOINTER)SQL_ASYNC_ENABLE_OFF, SQL_IS_INTEGER);
      if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
      async_enabled_ = false;
    }
  ...
  bool async_execute_direct(class connection& conn, void* event_handle, string const& query,
...
#endif
  result execute_direct(class connection& conn, string const& query, long batch_operations,
  ...
  RETCODE just_execute_direct(class connection& conn, string const& query, long batch_operations,
                              long timeout,
                              statement&,  // statement
                              void* event_handle = nullptr)
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_, SQL_ATTR_PARAMSET_SIZE,
    ...
    NANODBC_CALL_RC(NANODBC_FUNC(SQLExecDirect), rc, stmt_, (NANODBC_SQLCHAR*)query.c_str(),
                    SQL_NTS);
  ...
  result procedure_columns(string const& catalog, string const& schema, string const& procedure,
                           string const& column, statement& statement)
  {
    if (!open())
      throw programming_error("statement has no associated open connection");
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLProcedureColumns), rc, stmt_,
  ...
  unsigned long parameter_size(short param_index) const
  {
    ...
    NANODBC_CALL_RC(SQLDescribeParam, rc, stmt_, static_cast<SQLUSMALLINT>(param_index + 1),
                    &data_type, &parameter_size, nullptr, &nullable);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
    NANODBC_ASSERT(parameter_size <
  ...
  void prepare_bind(short param_index, std::size_t batch_size, param_direction direction,
                    bound_parameter& param)
  {
    ...
    if (!param_descr_data_.count(param_index))
    {
      ...
      NANODBC_CALL_RC(SQLDescribeParam, rc, stmt_, static_cast<SQLUSMALLINT>(param_index + 1),
                      &param_descr_data_[param_index].type_, &param_descr_data_[param_index].size_,
                      &param_descr_data_[param_index].scale_, &nullable);
      if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
    }
  ...
  template <class T>
  void bind(param_direction direction, short param_index, T const* values, std::size_t batch_size,
            bool const* nulls = nullptr, T const* null_sentry = nullptr);

  // handles multiple binary values
  void bind(param_direction direction, short param_index,
            std::vector<std::vector<uint8_t>> const& values, bool const* nulls = nullptr,
  ...
  void bind_strings(param_direction direction, short param_index, T const* values,
                    std::size_t value_size, std::size_t batch_size, bool const* nulls = nullptr,
                    T const* null_sentry = nullptr);

  template <class T, typename = enable_if_string<T>>
  void bind_strings(param_direction direction, short param_index, std::vector<T> const& values,
  ...
  void bind_null(short param_index, std::size_t batch_size)
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(SQLBindParameter, rc, stmt_,
                    param.index_ + 1,  // parameter number
  ...
  void describe_parameters(const std::vector<short>& idx, const std::vector<short>& type,
                           const std::vector<unsigned long>& size, const std::vector<short>& scale)
  {
...
template <class T>
void statement::statement_impl::bind(param_direction direction, short param_index, T const* values,
                                     std::size_t batch_size, bool const* nulls /*= nullptr*/,
                                     T const* null_sentry /*= nullptr*/)
{
  bound_parameter param;
  prepare_bind(param_index, batch_size, direction, param);
...
void statement::statement_impl::bind_strings(
    param_direction direction, short param_index, std::vector<T> const& values,
    bool const* nulls /*= nullptr*/, typename T::value_type const* null_sentry /*= nullptr*/)
{
  ...
  bind_strings(direction, param_index, string_data.data(), max_length, batch_size, nulls,
               null_sentry);
}

template <class T, typename>
void statement::statement_impl::bind_strings(param_direction direction, short param_index,
                                             T const* values, std::size_t value_size,
                                             ...
                                             T const* null_sentry /*= nullptr*/)
{
  ...
  if (null_sentry)
  {
    for (std::size_t i = 0; i < batch_size; ++i)
    {
      const std::basic_string<T> s_lhs(values + i * value_size, values + (i + 1) * value_size);
...
{
  return lhs.year == rhs.year && lhs.month == rhs.month && lhs.day == rhs.day &&
         lhs.hour == rhs.hour && lhs.min == rhs.min && lhs.sec == rhs.sec && lhs.fract == rhs.fract;
}

template <>
std::vector<wide_string::value_type>& statement::statement_impl::get_bound_string_data(
    short param_index)
{
...
template <>
std::vector<std::string::value_type>& statement::statement_impl::get_bound_string_data(
    short param_index)
{
...
public:
  ...
  void open(table_valued_parameter& tvp, statement& stmt, short param_index, std::size_t row_count)
  {
    bound_parameter param;
    ...
    *(SQLLEN*)bind_len_or_null->data() = 0 < row_count ? row_count : SQL_DEFAULT_PARAM;
    ...
    NANODBC_CALL_RC(SQLBindParameter, rc, hstmt, static_cast<SQLUSMALLINT>(param_index + 1),
                    SQL_PARAM_INPUT, SQL_C_DEFAULT, SQL_SS_TABLE, row_count, 0,
                    (SQLPOINTER)tvp_name.data(), SQL_NTS, bind_len_or_null->data());
    ...
    // set param focus
    NANODBC_CALL_RC(NANODBC_FUNC(SQLSetStmtAttr), rc, hstmt, SQL_SOPT_SS_PARAM_FOCUS,
                    (SQLPOINTER)(std::intptr_t)(param_index + 1), SQL_IS_INTEGER);
    if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
      NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);
  }
  ...
  void close()
  {
    ...
    if (open_)
    {
      ...
      {
        // reset param focus
        NANODBC_CALL_RC(NANODBC_FUNC(SQLSetStmtAttr), rc, stmt_impl->native_statement_handle(),
                        SQL_SOPT_SS_PARAM_FOCUS, (SQLPOINTER) nullptr, SQL_IS_INTEGER);
        if (!success(rc))
  ...
  void prepare_tvp_name()
  {
    ...
    // get ipd handle
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetStmtAttr), rc, hstmt, SQL_ATTR_IMP_PARAM_DESC, &hipd,
                    SQL_IS_POINTER, &str_len);
    if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc)
      NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);

    // get tvp name buffer length
    NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, hipd, param_index_ + 1, SQL_CA_SS_TYPE_NAME,
                    nullptr, 0, &buf_len);
    if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc)
  ...
  void prepare_tvp_param_all()
  {
    ...
    NANODBC_CALL_RC(NANODBC_FUNC(SQLSetStmtAttr), rc, hstmt, SQL_SOPT_SS_NAME_SCOPE,
                    (SQLPOINTER)SQL_SS_NAME_SCOPE_TABLE_TYPE, SQL_IS_UINTEGER);
    if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc)
      NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);

    NANODBC_CALL_RC(NANODBC_FUNC(SQLColumns), rc, hstmt, nullptr, 0, nullptr, 0,
                    (NANODBC_SQLCHAR*)tvp_name_.data(), SQL_NTS, nullptr, 0);
    if (SQL_SUCCESS != rc && SQL_SUCCESS_WITH_INFO != rc)
      NANODBC_THROW_DATABASE_ERROR(hstmt, SQL_HANDLE_STMT);
  ...
  template <class T>
  void bind(short param_index, T const* values, std::size_t batch_size, bool const* nulls = nullptr,
            T const* null_sentry = nullptr);

  void bind(short param_index, std::vector<std::vector<uint8_t>> const& values,
            bool const* nulls = nullptr, uint8_t const* null_sentry = nullptr)
  {
  ...
  template <class T, typename = enable_if_character<T>>
  void bind_strings(short param_index, T const* values, std::size_t value_size,
                    std::size_t batch_size, bool const* nulls = nullptr,
                    T const* null_sentry = nullptr);

  template <class T, typename = enable_if_string<T>>
  void bind_strings(short param_index, std::vector<T> const& values, bool const* nulls = nullptr,
                    typename T::value_type const* null_sentry = nullptr);
  ...
  void bind_null(short param_index)
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(SQLBindParameter, rc, stmt_impl->native_statement_handle(),
                    param.index_ + 1,  // parameter number
                    param.iotype_,     // input or output typ,
                    SQL_C_CHAR,
                    param.type_,  // parameter type
...
template <class T>
void table_valued_parameter::table_valued_parameter_impl::bind(short param_index, T const* values,
                                                               std::size_t batch_size,
                                                               bool const* nulls /*= nullptr*/,
                                                               T const* null_sentry /*= nullptr*/)
{
...
void table_valued_parameter::table_valued_parameter_impl::bind_strings(
    short param_index, std::vector<T> const& values, bool const* nulls /*= nullptr*/,
    typename T::value_type const* null_sentry /*= nullptr*/)
{
  if (values.size() < row_count_)
    throw programming_error("invalid values.size()");
...
template <class T, typename>
void table_valued_parameter::table_valued_parameter_impl::bind_strings(
    short param_index, T const* values, std::size_t value_size, std::size_t batch_size,
    bool const* nulls /*= nullptr*/, T const* null_sentry /*= nullptr*/)
{
  if (batch_size < row_count_)
    throw programming_error("invalid batch_size");
...
template <>
bool table_valued_parameter::table_valued_parameter_impl::equals(std::string const& lhs,
...
template <>
bool table_valued_parameter::table_valued_parameter_impl::equals(const wide_string& lhs,
...
template <>
bool table_valued_parameter::table_valued_parameter_impl::equals(const timestamp& lhs,
                                                                 const timestamp& rhs)
{
  return lhs.year == rhs.year && lhs.month == rhs.month && lhs.day == rhs.day &&
         lhs.hour == rhs.hour && lhs.min == rhs.min && lhs.sec == rhs.sec && lhs.fract == rhs.fract;
...
public:
  ...
  {
    RETCODE rc;
    NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROW_ARRAY_SIZE,
                    (SQLPOINTER)(std::intptr_t)rowset_size_, 0);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);

    NANODBC_CALL_RC(SQLSetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROWS_FETCHED_PTR,
                    &row_count_, 0);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
  ...
  unsigned long position() const
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(SQLGetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROW_NUMBER, &pos,
  ...
  string column_datatype_name(short column) const
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(NANODBC_FUNC(SQLColAttribute), rc, stmt_.native_statement_handle(),
                    static_cast<SQLUSMALLINT>(column + 1), SQL_DESC_TYPE_NAME, type_name,
                    sizeof(type_name), &len, nullptr);
    if (!success(rc))
      NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
...
private:
  ...
  void auto_bind_columns()
  {
    ...
    {
      NANODBC_CALL_RC(NANODBC_FUNC(SQLDescribeCol), rc, stmt_.native_statement_handle(),
                      static_cast<SQLUSMALLINT>(i + 1), (NANODBC_SQLCHAR*)column_name,
                      sizeof(column_name) / sizeof(NANODBC_SQLCHAR), &len, &sqltype, &sqlsize,
                      &scale, &nullable);
      if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
  ...
  void bind_column(bound_column& column)
  {
    ...
    RETCODE rc;
    NANODBC_CALL_RC(SQLBindCol, rc, stmt_.native_statement_handle(),
...
inline void result::result_impl::get_ref_impl(short column, T& result) const
{
  ...
  switch (col.ctype_)
  {
      ...
      if (!is_bound(column))
      {
        ...
        do
        {
          ...
          if (ValueLenOrInd == SQL_NO_TOTAL)
            ...
            out.append(buffer, std::min<std::size_t>(ValueLenOrInd, col.ctype_ == SQL_C_BINARY
                                                                        ? buffer_size
                                                                        : buffer_size - 1));
    ...
    case SQL_C_WCHAR:
    {
      if (!is_bound(column))
      {
        ...
        do
        {
          ...
          if (ValueLenOrInd == SQL_NO_TOTAL)
            ...
            out.append(buffer, std::min<std::size_t>(ValueLenOrInd / sizeof(wide_char_t),
      ...
      {  // bound and not blob
        SQLWCHAR const* s = reinterpret_cast<SQLWCHAR*>(col.pdata_ + rowset_position_ * col.clen_);
        string::size_type const str_size =
            col.cbdata_[static_cast<size_t>(rowset_position_)] / sizeof(SQLWCHAR);
        auto const us =
            reinterpret_cast<wide_char_t const*>(s);  // no-op or unsigned short to signed char16_t
    ...
    case SQL_C_DOUBLE:
    {
      ...
      const int bytes =
          std::snprintf(const_cast<char*>(buffer.data()), width + 1,
...
inline void result::result_impl::get_ref_impl<_variant_t>(short column, _variant_t& result) const
{
  ...
  switch (c_type)
  {
    ...
    case SQL_C_TYPE_TIME:
    {
      ...
      get_ref_impl(column, v);
      ::SYSTEMTIME st{
          0, 0, 0, 0, static_cast<WORD>(v.hour), static_cast<WORD>(v.min), static_cast<WORD>(v.sec),
          0};
      ::DATE date{0};
      if (!::SystemTimeToVariantTime(&st, &date))
        throw type_incompatible_error();
    ...
    case SQL_C_TYPE_TIMESTAMP:
    {
      ...
      get_ref_impl(column, v);
      SYSTEMTIME st{
          static_cast<WORD>(v.year), static_cast<WORD>(v.month),          0,
          static_cast<WORD>(v.day),  static_cast<WORD>(v.hour),           static_cast<WORD>(v.min),
          static_cast<WORD>(v.sec),  static_cast<unsigned short>(v.fract)};
      DATE date;
      if (!::SystemTimeToVariantTime(&st, &date))
        throw type_incompatible_error();
      result = _variant_t(date, VT_DATE);
...
std::unique_ptr<T, std::function<void(T*)>> result::result_impl::ensure_pdata(short column) const
{
  ...
  if (is_bound(column))
  {
    ...
    return std::unique_ptr<T, std::function<void(T*)>>(
        (T*)(col.pdata_ + rowset_position_ * col.clen_),
        [](T*)
        {
        });
  }
...
std::list<datasource> list_datasources()
{
  ...
  do
  {
    NANODBC_CALL_RC(NANODBC_FUNC(SQLDataSources), rc,
    ...
    if (rc == SQL_SUCCESS)
    {
      ...
      dsn.driver = string(&driver[0], &driver[std::char_traits<NANODBC_SQLCHAR>::length(driver)]);
...
std::list<driver> list_drivers()
{
  ...
  do
  {
    NANODBC_CALL_RC(NANODBC_FUNC(SQLDrivers), rc, env.native_env_handle(),
...
#ifdef NANODBC_HAS_STD_VARIANT
connection::connection(string const& dsn, string const& user, string const& pass,
                       std::list<attribute> const& attributes)
...
#ifdef NANODBC_HAS_STD_VARIANT
void connection::connect(string const& dsn, string const& user, string const& pass,
                         std::list<attribute> const& attributes)
{
...
#if !defined(NANODBC_DISABLE_ASYNC) && defined(SQL_ATTR_ASYNC_DBC_EVENT)
bool connection::async_connect(string const& dsn, string const& user, string const& pass,
                               void* event_handle, long timeout)
{
...
result statement::execute_direct(class connection& conn, string const& query, long batch_operations,
                                 long timeout)
{
...
bool statement::async_execute_direct(class connection& conn, void* event_handle,
                                     string const& query, long batch_operations, long timeout)
{
...
void statement::just_execute_direct(class connection& conn, string const& query,
                                    long batch_operations, long timeout)
{
...
result statement::procedure_columns(string const& catalog, string const& schema,
                                    string const& procedure, string const& column)
{
...
#define NANODBC_INSTANTIATE_BINDS(type)                                                        \
  ...
  template void statement::bind(short, const type*, std::size_t, const type*,                  \
                                param_direction); /* n-ary, sentry */                          \
  template void statement::bind(short, const type*, std::size_t, const bool*,                  \
...
#define NANODBC_INSTANTIATE_BIND_VECTOR_STRINGS(type)                                             \
  ...
  template void statement::bind_strings(short, std::vector<type> const&, type::value_type const*, \
                                        param_direction);                                         \
  template void statement::bind_strings(short, std::vector<type> const&, bool const*,             \
                                        param_direction);

#define NANODBC_INSTANTIATE_BIND_STRINGS(type)                                                    \
  NANODBC_INSTANTIATE_BIND_VECTOR_STRINGS(type)                                                   \
  template void statement::bind_strings(short, const type::value_type*, std::size_t, std::size_t, \
                                        param_direction);                                         \
  template void statement::bind_strings(short, type::value_type const*, std::size_t, std::size_t, \
                                        type::value_type const*, param_direction);                \
  template void statement::bind_strings(short, type::value_type const*, std::size_t, std::size_t, \
                                        bool const*, param_direction)
...
template <class T>
void statement::bind(short param_index, T const* values, std::size_t batch_size,
...
template <class T>
void statement::bind(short param_index, T const* values, std::size_t batch_size,
                     T const* null_sentry, param_direction direction)
{
  impl_->bind(direction, param_index, values, batch_size, nullptr, null_sentry);
}

template <class T>
void statement::bind(short param_index, T const* values, std::size_t batch_size, bool const* nulls,
                     param_direction direction)
{
  impl_->bind(direction, param_index, values, batch_size, nulls);
}

void statement::bind(short param_index, std::vector<std::vector<uint8_t>> const& values,
                     param_direction direction)
{
...
void statement::bind(short param_index, std::vector<std::vector<uint8_t>> const& values,
                     bool const* nulls, param_direction direction)
{
  impl_->bind(direction, param_index, values, nulls);
}

void statement::bind(short param_index, std::vector<std::vector<uint8_t>> const& values,
                     uint8_t const* null_sentry, param_direction direction)
{
  impl_->bind(direction, param_index, values, nullptr, null_sentry);
}

template <class T, typename>
void statement::bind_strings(short param_index, std::vector<T> const& values,
                             param_direction direction)
{
...
template <class T, typename>
void statement::bind_strings(short param_index, T const* values, std::size_t value_size,
                             std::size_t batch_size, param_direction direction)
{
...
template <class T, typename>
void statement::bind_strings(short param_index, T const* values, std::size_t value_size,
                             std::size_t batch_size, T const* null_sentry,
                             param_direction direction)
{
...
template <class T, typename>
void statement::bind_strings(short param_index, T const* values, std::size_t value_size,
                             std::size_t batch_size, bool const* nulls, param_direction direction)
{
...
template <class T, typename>
void statement::bind_strings(short param_index, std::vector<T> const& values,
                             typename T::value_type const* null_sentry, param_direction direction)
{
  impl_->bind_strings(direction, param_index, values, nullptr, null_sentry);
}

template <class T, typename>
void statement::bind_strings(short param_index, std::vector<T> const& values, bool const* nulls,
                             param_direction direction)
{
...
void statement::describe_parameters(const std::vector<short>& idx, const std::vector<short>& type,
...
implementation_row_descriptor::sql_get_descr_field::sql_get_descr_field(
    implementation_row_descriptor const& ird, short record, std::uint16_t field_identifier) noexcept
    : ird_(ird),
      record_(record),
...
implementation_row_descriptor::sql_get_descr_field::operator std::int64_t() const
{
  ...
  SQLULEN value = 0;
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, ird_.descriptor_handle_,
                  static_cast<SQLUSMALLINT>(record_ + 1), field_identifier_, &value, 0, nullptr);
  if (!success(rc))
...
implementation_row_descriptor::sql_get_descr_field::operator string() const
{
  ...
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, ird_.descriptor_handle_,
                  static_cast<SQLUSMALLINT>(record_ + 1), field_identifier_, value,
                  sizeof(value) / sizeof(NANODBC_SQLCHAR), &len);
  if (!success(rc))
    NANODBC_THROW_DATABASE_ERROR(ird_.statement_handle_, SQL_HANDLE_STMT);
...
{
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLGetStmtAttr), rc, statement_handle_, SQL_ATTR_IMP_ROW_DESC,
                  &descriptor_handle_, 0, nullptr);
  if (!success(rc))
    NANODBC_THROW_DATABASE_ERROR(statement_handle_, SQL_HANDLE_STMT);

  NANODBC_CALL_RC(NANODBC_FUNC(SQLGetDescField), rc, descriptor_handle_, 0, SQL_DESC_COUNT,
                  &descriptor_records_count_, 0, nullptr);
  if (!success(rc))
    NANODBC_THROW_DATABASE_ERROR(statement_handle_, SQL_HANDLE_STMT);
}
...
#define NANODBC_INSTANTIATE_TVP_BINDS(type)                                                \
  ...
  template void table_valued_parameter::bind(short, const type*, std::size_t,              \
                                             const type*); /* n-ary, sentry */             \
  template void table_valued_parameter::bind(short, const type*, std::size_t,              \
                                             const bool*) /* n-ary, flags */
...
#define NANODBC_INSTANTIATE_TVP_BIND_VECTOR_STRINGS(type)                              \
  ...
  template void table_valued_parameter::bind_strings(short, std::vector<type> const&, bool const*);

#define NANODBC_INSTANTIATE_TVP_BIND_STRINGS(type)                                                \
  NANODBC_INSTANTIATE_TVP_BIND_VECTOR_STRINGS(type)                                               \
  template void table_valued_parameter::bind_strings(short, const type::value_type*, std::size_t, \
                                                     std::size_t);                                \
  template void table_valued_parameter::bind_strings(short, type::value_type const*, std::size_t, \
                                                     std::size_t, type::value_type const*);       \
  template void table_valued_parameter::bind_strings(short, type::value_type const*, std::size_t, \
...
void table_valued_parameter::bind(short param_index,
...
template <class T, typename>
void table_valued_parameter::bind_strings(short param_index, T const* values,
                                          std::size_t value_size, std::size_t batch_size)
{
...
void table_valued_parameter::describe_parameters(const std::vector<short>& idx,
...
catalog::tables catalog::find_tables(string const& table, string const& type, string const& schema,
...
catalog::procedures catalog::find_procedures(string const& procedure, string const& schema,
                                             string const& catalog)
{
  // Passing a null pointer to a search pattern argument does not
  // constrain the search for that argument; that is, a null pointer and
  ...
  statement stmt(conn_);
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLProcedures), rc, stmt.native_statement_handle(),
                  (NANODBC_SQLCHAR*)(catalog.empty() ? nullptr : catalog.c_str()),
                  (catalog.empty() ? 0 : SQL_NTS),
                  (NANODBC_SQLCHAR*)(schema.empty() ? nullptr : schema.c_str()),
...
catalog::procedure_columns catalog::find_procedure_columns(string const& column,
                                                           ...
                                                           string const& catalog)
{
  statement stmt(conn_);
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLProcedureColumns), rc, stmt.native_statement_handle(),
                  (NANODBC_SQLCHAR*)(catalog.empty() ? nullptr : catalog.c_str()),
                  (catalog.empty() ? 0 : SQL_NTS),
                  (NANODBC_SQLCHAR*)(schema.empty() ? nullptr : schema.c_str()),
                  (schema.empty() ? 0 : SQL_NTS),
...
catalog::table_privileges catalog::find_table_privileges(string const& catalog, string const& table,
                                                         string const& schema)
{
  // Passing a null pointer to a search pattern argument does not
  // constrain the search for that argument; that is, a null pointer and
...
catalog::columns catalog::find_columns(string const& column, string const& table,
                                       string const& schema, string const& catalog)
{
  statement stmt(conn_);
  RETCODE rc;
  NANODBC_CALL_RC(
...
catalog::primary_keys catalog::find_primary_keys(string const& table, string const& schema,
                                                 string const& catalog)
{
  statement stmt(conn_);
  RETCODE rc;
...
std::list<string> catalog::list_catalogs()
{
  ...
  statement stmt(conn_);
  RETCODE rc;
  NANODBC_CALL_RC(NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(),
...
std::list<string> catalog::list_table_types()
{
  ...
  NANODBC_CALL_RC(NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(),
                  (NANODBC_SQLCHAR*)NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*)NANODBC_TEXT(""), 0,
                  (NANODBC_SQLCHAR*)NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR*)SQL_ALL_TABLE_TYPES, 1);
  if (!success(rc))
    NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
...
template void result::get_ref(short, const std::string::value_type&,
                              std::string::value_type&) const;
template void result::get_ref(short, const wide_string::value_type&,
                              wide_string::value_type&) const;
...
template void result::get_ref(short, const std::vector<std::uint8_t>&,
                              std::vector<std::uint8_t>&) const;
#if defined(_MSC_VER)
template void result::get_ref(short, _variant_t const&, _variant_t&) const;
#endif

template void result::get_ref(string const&, const std::string::value_type&,
                              std::string::value_type&) const;
template void result::get_ref(string const&, const wide_string::value_type&,
                              wide_string::value_type&) const;
...
template void result::get_ref(string const&, const unsigned long long int&,
...
template void result::get_ref(string const&, const std::vector<std::uint8_t>&,
                              std::vector<std::uint8_t>&) const;
#if defined(_MSC_VER)
...
template std::vector<std::uint8_t> result::get(string const&,
