...
public:
  ...
  /**
   * @param src_buf 전송할 내용
   * @param dest_buf 목적지 버퍼는 미리 sbuf.size() + length_field_size만큼 확보해야 함
   * @param header_only true이면 길이 필드만 dest_buf의 앞에 쓴다.
   * @return src_buf.size() + length_field_size를 돌려줌
   */
  size_t encode(const const_buffer& src_buf, mutable_buffer& dest_buf, bool header_only = false);
