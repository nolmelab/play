...
template <typename Topic, typename Frame> class protocol
{
public:
  using frame = Frame;
  ...
  using recv_fn =
      std::function<void(Topic topic, const void *data, size_t len)>;
...
protected:
  void send(const void *data, size_t len) { send_fn_(data, len); }
