...
#define observer_ptr_VERSION                                                                    \
  nsop_STRINGIFY(observer_ptr_MAJOR) "." nsop_STRINGIFY(observer_ptr_MINOR) "." nsop_STRINGIFY( \
      observer_ptr_PATCH)
...
#else
#define nsop_CONFIG_ALLOW_IMPLICIT_CONVERSION_FROM_UNIQUE_PTR \
  nsop_CONFIG_ALLOW_IMPLICIT_CONVERSION_FROM_SMART_PTR
#define nsop_CONFIG_ALLOW_IMPLICIT_CONVERSION_FROM_SHARED_PTR \
  nsop_CONFIG_ALLOW_IMPLICIT_CONVERSION_FROM_SMART_PTR
#endif
...
#define nsop_CONFIG_SELECT_OBSERVER_PTR \
  (nsop_HAVE_STD_OBSERVER_PTR ? nsop_OBSERVER_PTR_STD : nsop_OBSERVER_PTR_NONSTD)
#endif
...
#define nsop_USES_STD_OBSERVER_PTR                               \
  ((nsop_CONFIG_SELECT_OBSERVER_PTR == nsop_OBSERVER_PTR_STD) || \
   ((nsop_CONFIG_SELECT_OBSERVER_PTR == nsop_OBSERVER_PTR_DEFAULT) && nsop_HAVE_STD_OBSERVER_PTR))
...
#define nsop_COMPILER_CLANG_VERSION \
  nsop_COMPILER_VERSION(__clang_major__, __clang_minor__, __clang_patchlevel__)
#else
#define nsop_COMPILER_CLANG_VERSION 0
#endif

#if defined(__GNUC__) && !defined(__clang__)
#define nsop_COMPILER_GNUC_VERSION \
  nsop_COMPILER_VERSION(__GNUC__, __GNUC_MINOR__, __GNUC_PATCHLEVEL__)
#else
...
#define nsop_HAVE_IMPLICIT_CONVERSION_FROM_SMART_PTR                                     \
  (nsop_HAVE_STD_SMART_PTRS && (nsop_CONFIG_ALLOW_IMPLICIT_CONVERSION_FROM_UNIQUE_PTR || \
...
#define nsop_HAVE_OWN_COMMON_TYPE_STD \
  (nsop_HAVE_STD_DECAY && nsop_HAVE_STD_DECLVAL || nsop_HAVE_TYPEOF)
#define nsop_HAVE_OWN_COMMON_TYPE_TYPEOF nsop_HAVE_TYPEOF

// Method enabling

#define nsop_REQUIRES_T(VA) , typename std::enable_if<(VA), int>::type = 0
...
public:
...
#if nsop_HAVE_NULLPTR
  nsop_constexpr14 observer_ptr(std::nullptr_t) nsop_noexcept : ptr(nullptr) {}
#endif

  nsop_constexpr14 explicit observer_ptr(pointer p) nsop_noexcept : ptr(p) {}
...
#endif
            >
  nsop_constexpr14 observer_ptr(observer_ptr<W2> other) nsop_noexcept : ptr(other.get())
  {
  }

#if nsop_CONFIG_ALLOW_IMPLICIT_CONVERSION_FROM_UNIQUE_PTR && nsop_HAVE_STD_SMART_PTRS
  template <class W2 nsop_REQUIRES_T((std::is_convertible<W2*, W*>::value))>
  nsop_constexpr14 observer_ptr(std::unique_ptr<W2> const& other) nsop_noexcept : ptr(other.get())
  {
  }
#endif
  ...
  template <class W2 nsop_REQUIRES_T((std::is_convertible<W2*, W*>::value))>
  nsop_constexpr14 observer_ptr(std::shared_ptr<W2> const& other) nsop_noexcept : ptr(other.get())
  {
  }
#endif
